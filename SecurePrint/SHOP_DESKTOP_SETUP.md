...
- **History & Payments (M11)**: View paginated history of shop jobs, along with a detailed status timeline per job. Securely mark `PAY_AT_SHOP` jobs as Paid locally.
- **Shop Identity QR (M11)**: Dedicated profile tab separating the Shop Identification QR natively from the Secure Document Access QR scanner.
...
## 14. Current Limitations (M11 Complete)
*   No final installer (`setup.exe`) is created yet (Planned for M12).
